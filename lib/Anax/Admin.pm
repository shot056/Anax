...
sub do_login {
    ...
    my %h;
    $h{ "id :" . $params->{login_id} . " eq " . $self->app->config->{login_id} } = $params->{login_id} eq $self->app->config->{login_id};
    $h{ "pwd:" . $params->{password} . " eq " .  $self->app->config->{password} } = $params->{password} eq $self->app->config->{password};
    $self->app->dumper( \%h );
